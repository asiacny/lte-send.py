...
logfile = '/mnt/nfs/mycloud/mobilesvr/logs/lte_modem_send.py.log'
sendlist = '/mnt/nfs/mycloud/mobilesvr/logs/text-message-sent-list.txt'
...
dbpath = '/mnt/nfs/mycloud/mobilesvr/database/'
...
"""
def create_message(recipient, message, image='', title='', altmsg='', img_userpass='', img_delay=0, img_qty=1, img_qty_delay=1):
    ...
    elif not message and not image:
    ...
    for temp in mobile_list:
        if not (10 <= len(temp) <= 11):
            output_close("Error: Phone number "+temp+" is too long or short")
    ...
    if image:
        ...
        for c in range(int(img_qty)):
            ...
            imagefile_holder = upload_file(image, False, clear_attachments, img_userpass, (c+1))
...
# Regular Args
#-----------------------------------------------------------
parser.add_argument('-r','--recipient', help=recipient_help, required=True)
...
"""
details_dict = create_message(args['recipient'], args['message'], args['image'], args['title'], args['altmsg'], args['imguserpass'], args['imgdelay'], args['imgqty'], args['imgqtydelay'])
